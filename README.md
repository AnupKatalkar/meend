...
<!-- The logo carries the wordmark, so it *is* the h1: its alt text gives the
     document a proper title without printing the name twice. -->
<h1><img src="docs/logo.png" alt="Meend" width="420" /></h1>

**Play chords with your hands. Nothing leaves your device**

A browser-only musical instrument played with hand gestures in front of a webcam.

[![Runs in your browser](https://img.shields.io/badge/runs-100%25_in_your_browser-4de3ff?style=flat-square)](#quick-start)
[![Camera data never uploaded](https://img.shields.io/badge/camera_data-never_uploaded-4de3ff?style=flat-square)](#why-this-exists)
[![Tests](https://img.shields.io/badge/tests-133_unit_%2B_67_browser-4de3ff?style=flat-square)](#testing)
[![Latency](https://img.shields.io/badge/gesture_to_sound-~28ms-ff5ec8?style=flat-square)](#testing)

[Why](#why-this-exists) · [Quick start](#quick-start) · [Playing it](#playing-it) ·
[Raga mode](#raga-mode) · [Architecture](#architecture) · [Testing](#testing)
...
shapes expression. Hold up three fingers and you hear a iii chord — no tutorial,
no MIDI controller, no install.
...
- **Five play modes** — gesture chords, theremin, mono piano, Hindustani raga,
  and a full keyboard fallback for when there is no camera.
- **Three instruments** — a detuned synth pad, an FM electric piano, and a
  sampled acoustic grand.
- **13 ragas, 6 talas** — modelled properly, with separate ascending and
  descending lines and a tanpura that retunes itself.
- **Record and share** — up to 30 seconds of audio, full video, or a
  skeleton-only capture that shows your hands but not your room.
